...
#define MAX_RCV_BUFFER_SIZE     20                                      /* Definition of the receive data           */
int f=0;
/*********************************************************************************************************************/
/*-------------------------------------------------Global variables--------------------------------------------------*/
/*********************************************************************************************************************/
...
uint8 g_rxData[SIZE] = {''};
uint8 g_rxData_a[SIZE] = {''};
uint8 g_rxData_b[SIZE] = {''};
uint8 g_rxData_c[SIZE] = {''};
uint8 g_rxRcvBuffer[MAX_RCV_BUFFER_SIZE] = {''};
int a=0;
/* Size of the message */
uint8 g_txData[] = "read  ";
uint8 ga_txData[] = "send  ";
Ifx_SizeT g_count = sizeof(g_txData);
Ifx_SizeT ga_count = sizeof(ga_txData);
int cnt_rcv = 0;
int c=0;
union
{
...
{
    int size_of_read = 1; /* 1 Byte Read */

    a=0;

    IfxAsclin_Asc_isrReceive(&g_ascHandle);
    IfxAsclin_Asc_read(&g_ascHandle,g_rxData,&size_of_read,TIME_INFINITE); /* Receive data via RX */
    ...
    if(cnt_rcv>=10 && f==1)
    {
        ...
        while(1)
        {
            ...
            crc_16_val.check_sum = CRC16_MODBUS(g_rxData_c,5);

            if(g_rxData_a[1]==crc_16_val.b[1] && g_rxData_a[0]==crc_16_val.b[0])
            {
                c=a;
                break;
            }
...
void send_ASCLIN_UART_message(int A)
{
    g_rxData_b[0] = 174;
    g_rxData_b[1] = 184;
    g_rxData_b[2] = 'I';
    g_rxData_b[3] = 01;
    g_rxData_b[4] = A;
    crc_16_val.check_sum = CRC16_MODBUS(g_rxData_b,5);

    //sprintf(g_txData_a,"%X",crc_16_val.b[0]);
    /*AA[0]=crc_16_val.b[0];
    AA[1]=crc_16_val.b[1];*/
    Z=crc_16_val.b[1];

    sprintf(X,"%X",crc_16_val.b[0]);
    Ifx_SizeT g_count_a = sizeof(X);
    IfxAsclin_Asc_write(&g_ascHandle,X, &g_count_a, TIME_INFINITE);
    sprintf(Y,"%X",Z);
    Ifx_SizeT g_count_b = sizeof(Y);
    IfxAsclin_Asc_write(&g_ascHandle,Y, &g_count_b, TIME_INFINITE);
...
int receive_ASCLIN_UART_message(sint32 cnt)
{
    /* Receive data via RX  */
